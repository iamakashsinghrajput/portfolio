...
"use client";
import React from "react";
import { StaticImageData } from "next/image"; // Import StaticImageData for type safety

import { Carousel, Card } from "@/src/components/ui/apple-cards-carousel"; // Adjust path if needed

// Import Image Assets (Ensure paths are correct)
import Weather from "@/src/assets/weather.png";
import Detective from "@/src/assets/detectivedev.png";
import tictactoe from "@/src/assets/tictactoe.png";
import Primedrive from "@/src/assets/PrimeDrive.png";
...
interface ProjectData {
    category: string;
    title: string;
    src: StaticImageData | string; // Allow both imported images and potentially string URLs
    contentData: ProjectDetailsProps; // Nested object for content details
}
...
interface ProjectDetailsProps {
    ...
    projectUrl: string;
    projectLinkText?: string; // Optional text for the link
}
...
const ProjectDetailsContent: React.FC<ProjectDetailsProps> = ({
    ...
    return (
        <div className="bg-[#F5F5F7] dark:bg-neutral-800 p-8 md:p-10 lg:p-14 rounded-3xl"> {/* Consistent Padding */}
            <div className="text-neutral-600 dark:text-neutral-400 text-base md:text-xl font-sans max-w-3xl mx-auto space-y-6"> {/* Adjusted Text Size & Spacing */}
                ...
                <div>
                    ...
                    <a
                        ...
                        className="text-blue-600 dark:text-blue-400 hover:underline inline-block" // Enhanced Link Styling
                    >
                        {projectLinkText || projectUrl} {/* Use custom text or default to URL */}
                    </a>
                </div>
            </div>
        </div>
...
export function AppleCardsCarouselDemo() {
    // Map data to Card components, passing the reusable content component
    const cards = projects.map((project, index) => (
        <Card
            key={project.category + index} // Use a more robust key
            card={{
                category: project.category,
                title: project.title,
                src: project.src,
                content: <ProjectDetailsContent {...project.contentData} /> // Pass structured data to the reusable component
            }}
            index={index}
        />
    ));
    ...
    return (
        // Removed height constraint, let content define height. Added padding.
