  ...
  return (
    <div className="mb-4 text-4xl font-bold text-neutrals-50 md:text-5xl">
      ...
      <AnimatePresence mode="wait">
        <motion.p key={currentHeading} variants={headingVariants} initial="enter" animate="visible" exit="exit">
          {currentHeading.split('').map((char, index) => (
            <motion.span key={`${char}-${index}`} className="inline-block whitespace-pre" variants={letterVariants}>
              {char}
            </motion.span>
          ))}
        </motion.p>
