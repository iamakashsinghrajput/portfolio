...
import { AnimatePresence, motion, useAnimation, useAnimate } from 'framer-motion';
import { useEffect } from 'react';
import { Container } from '../ui/container';

function MobileNavigation({ isOpen, onClose, links }: { isOpen: boolean; onClose: () => void; links: { label: string; href: string }[] }) {
            ...
            >
              {links.map((link, index) => (
                <a
                  ...
                  onClick={onClose}
                  className={cn(
                    'py-4 ps-2 uppercase text-white transition-[letter-spacing,color]',
...
function MobileNavigationToggle({
  ...
  return (
    <button
      ...
      className={cn('rounded-full px-3 py-1.5 transition-colors duration-500 md:px-4 md:py-2',
